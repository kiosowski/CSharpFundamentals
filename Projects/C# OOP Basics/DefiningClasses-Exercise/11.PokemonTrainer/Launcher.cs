...
{
    private const int PokemonHealthLossDueToMissingElement = 10;
    ...
    private static Queue<Trainer> GetTrainers()
    {
        ...
        var playerData = Console.ReadLine().Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries).Select(x => x.Trim()).ToArray();
        ...
        while (playerData[0] != "Tournament")
        {
            ...
            playerData = Console.ReadLine().Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries).Select(x => x.Trim()).ToArray();
        }
        return trainers;
    }
}
