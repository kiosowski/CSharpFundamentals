...
namespace _04.ShoppingSpree
{
    class StartUp
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                people = new Queue<Person>(Console.ReadLine().Split(new char[] { ';', ' ' }, StringSplitOptions.RemoveEmptyEntries).Select(p => p.Split('=')).Select(p => new Person(p[0], decimal.Parse(p[1]))));

                products = new Queue<Product>(Console.ReadLine().Split(new char[] { ';', ' ' }, StringSplitOptions.RemoveEmptyEntries).Select(p => p.Split('=')).Select(p => new Product(p[0], decimal.Parse(p[1]))));
            }
        ...
        {
            Console.WriteLine(string.Join(Environment.NewLine, people.Select(person => (person.Products.Count == 0) ? $"{person.Name} - Nothing bought" : $"{person.Name} - {string.Join(", ", person.Products.Select(product => product.Name))}")));
        }
