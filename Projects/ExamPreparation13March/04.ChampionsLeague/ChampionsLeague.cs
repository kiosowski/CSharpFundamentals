    ...
    class ChampionsLeague
    {
        ...
        private static HashSet<Team> GetTeams()
        {
            ...
            while (input[0] != "stop")
            {
                ...
                var firstHost = input[2].Split(new char[] { ':' }, StringSplitOptions.RemoveEmptyEntries).Select(int.Parse).ToArray();
                var secondHost = input[3].Split(new char[] { ':' }, StringSplitOptions.RemoveEmptyEntries).Select(int.Parse).ToArray();
